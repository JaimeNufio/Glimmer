...
local function fn()
    ...
    inst.components.lootdropper:SetChanceLootTable('glimmer') 
    ...
    inst.components.periodicspawner.basetime = 3 * rdrop * TUNING.TOTAL_DAY_TIME / (rec)
    inst.components.periodicspawner.randtime = 3 * rdrop * TUNING.TOTAL_DAY_TIME / (3*rec)
	...
	--inst.task = inst:DoPeriodicTask(TUNING.TOTAL_DAY_TIME*1/70, function() OnSpawnFuel(inst, SpawnPrefab("taffy")) end)
...
return Prefab("common/creatures/glimmer", fn, assets, prefabs)
